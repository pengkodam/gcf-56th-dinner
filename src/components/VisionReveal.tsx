...
export default function VisionReveal() {
  return (
    <section className="relative py-20 sm:py-28 bg-darker overflow-hidden">
      {/* Radial glow */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="w-[600px] h-[300px] bg-gold/4 rounded-full blur-[100px]" />
      </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-x-3 sm:gap-x-4 gap-y-2">
          {words.map((word, i) => (
            <motion.span
              ...
              className="font-display text-2xl sm:text-4xl lg:text-5xl font-bold text-white/90"
            >
              {word === "Christ!" ? (
                <span className="text-gold shimmer-text">{word}</span>
              ) : (
                word
              )}
            </motion.span>
        ...
        <ScrollReveal delay={0.8}>
          <p className="mt-10 text-white/40 text-sm sm:text-base max-w-2xl mx-auto leading-relaxed">
